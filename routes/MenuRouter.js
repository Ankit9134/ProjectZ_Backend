...
// Update tiffin details by owner email
router.put("/tiffin/:email", async (req, res) => {
  const ownerMailParam = req.params.email;
  let tiffinId = null;
...
// Edit an existing Meal Plan label
...
// Manage service days and flexible dates setting
...
// Apply meal plans to a specific meal type
...
router.put("/edit-meal-type/:mealTypeId/:email", async (req, res) => {
  ...
  try {
    ...
    if (!label || !description || !prices || typeof prices !== "object") {
      ...
      return res.status(400).json({
        message: "Meal type label, description and prices object are required.",
      });
    }
    if (applyTo && applyTo === "specific" && !Array.isArray(selectedPlans)) {
      historyLogRecorder(
...
router.delete("/delete-plan/:planId/:email", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Plan deleted successfully.", tiffinMenu: tiffin.menu }); // Return updated menu
  } catch (error) {
    historyLogRecorder(
      req,
      "Tiffin",
...
router.put("/edit-instruction/:id/:email", async (req, res) => {
  const { id: instructionId, email } = req.params;
  const { title, details } = req.body;
  let tiffinId = null;
