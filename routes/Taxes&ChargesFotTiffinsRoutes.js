...
const Tiffin = require("../models/Tiffin"); // Adjust path
const historyLogRecorder = require("../utils/historyLogRecorder.js"); // Adjust path
...
router.post("/add-taxes", async (req, res) => {
  ...
  try {
    ...
    if (!tiffin.tax || tiffin.tax.length === 0) {
      ...
      // Save immediately after initializing defaults if needed, or wait for adding new tax
      // await tiffin.save(); // Consider if needed here or combine saves
    }

    // Check if custom tax with the same name already exists
    ...
    // Only allow adding new custom taxes via this route
    const newTax = {
      name,
      rate: parseFloat(rate), // Ensure rate is a number
      isApplicable: isApplicable ?? true, // Default to true if not provided
      isDefault: false, // Custom taxes are never default
    };
...
router.get("/get-taxes/:email", async (req, res) => {
  ...
  let tiffinId = null;
  try {
    let tiffin = await Tiffin.findOne({ ownerMail: email }).lean(); // Use lean
    if (!tiffin) {
      historyLogRecorder(
        req,
        "Tiffin",
    ...
    let defaultsInitialized = false;
    // Initialize with default taxes if no taxes exist (this path should ideally not modify data in a GET request)
    // Consider initializing defaults upon Tiffin creation or first update instead.
    if (!taxes || taxes.length === 0) {
      taxes = [...defaultTaxes];
      // This save operation in a GET request is generally discouraged.
      // await Tiffin.updateOne({ _id: tiffinId }, { $set: { tax: defaultTaxes } });
      // defaultsInitialized = true; // Indicate if defaults were just set conceptually
      console.warn(
...
// Update tax details for a specific Tiffin tax subdocument
...
router.delete("/delete-taxes/:id/:email", async (req, res) => {
  ...
  try {
    ...
    const taxToDelete = tiffin.tax.find((tax) => tax._id.toString() === taxId);
    if (!taxToDelete) {
      historyLogRecorder(
        req,
        "Tiffin",
...
router.delete("/delete-charges/:id/:email", async (req, res) => {
  ...
  try {
    ...
    if (updateResult.modifiedCount === 0) {
      ...
      return res.status(404).json({
        message: "Charge could not be removed (may have been deleted already).",
      });
    }
...
router.put("/delivery-ranges/:id", async (req, res) => {
  ...
  const { email, minDistance, maxDistance, charge } = req.body; // Tiffin email needed to find the parent doc
  let tiffinId = null;
...
router.patch("/delivery-ranges/:id/:email", async (req, res) => {
  ...
  try {
    ...
    if (isActive === undefined || typeof isActive !== "boolean") {
      ...
      return res.status(400).json({
        message: "A boolean 'isActive' field is required in the request body.",
      });
    }
