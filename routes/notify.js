...
router.get("/restaurant", async (req, res) => {
  try {
    const data = await Notify.find({type:'restaurant'}).sort({ timestamp: -1 });;
    res.json({ success: true, data });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.get("/marketing", async (req, res) => {
  try {
    const data = await Notify.find({type:'marketing'}).sort({ timestamp: -1 });;
    res.json({ success: true, data });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
...
router.delete("/category/:category", async (req, res) => {
  try {
    ...
    const deletedNotifications = await Notify.deleteMany({ "metadata.category": category });
    ...
    res.status(500).json({ response: "error", success: false, message: err.message });
  }
});
