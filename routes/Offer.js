...
// Get all offers for a tiffin
router.get("/:mail/offers", async (req, res) => {
  const ownerMail = req.params.mail;
  try {
    const tiffin = await Tiffin.findOne({ ownerMail: ownerMail }).lean(); // Use lean for read-only
    if (!tiffin) {
      historyLogRecorder(
        req,
        "Tiffin",
...
router.post("/:mail/offers", async (req, res) => {
  const ownerMail = req.params.mail;
  let tiffinId = null;
  let offerCode = "Unknown"; // For logging context
  try {
